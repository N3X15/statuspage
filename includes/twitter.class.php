...
class Twitter
{
	...
	public static function canPostToTwitter() {
		...
		&& !empty($config['twitter_oauth_token_secret']);
	}
	...
	function __construct()
	{
		...
		if (empty($config['twitter_key']) || empty($config['twitter_secret']))
			throw new Exception('Twitter consumer key and secret must be specified in the configuration file');
		if (empty($config['twitter_oauth_token']) || empty($config['twitter_oauth_token_secret']))
			throw new Exception('Twitter token and secret must be specified in the configuration file');
	...
	{
		if (empty($message))
			throw new Exception('You must specify a message to tweet');

		if ($this->testing)
			$message = '(TESTING ONLY) ' . $message;
		$message = $this->prepareMessage($message);

		$result = $this->twitterObj->post('statuses/update', array('status' => $message));

		if ($this->testing)
			echo "Twitter result was:";
		if ($this->testing)
			print_r($result);
		if ($this->testing)
			echo "\n";
	...
	private function prepareMessage($message, $limit = 137)
	{
		...
		if ($this->testing)
			echo "Final twitter message: {$message}\n";
